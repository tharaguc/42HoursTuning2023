...
import { MatchGroupDetail, User } from "../../model/types";
...
export const insertMatchGroup = async (matchGroupDetail: MatchGroupDetail) => {
  ...
  const members = matchGroupDetail.members;
  const memberValues = members.map((member) => [matchGroupDetail.matchGroupId, member.userId]);
...
export const getMatchGroupDetailsByUserId = async (
  userId: string,
  status?: string
  ...
  const [matchGroupRows] = await pool.query<RowDataPacket[]>(query, [userId]);

  const matchGroupDetails: MatchGroupDetail[] = [];

  for (const matchGroup of matchGroupRows) {
    const [matchGroupMemberIdRows] = await pool.query<RowDataPacket[]>(
      "SELECT user_id FROM match_group_member WHERE match_group_id = ?",
      [matchGroup.match_group_id]
    );
    ...
    matchGroup.members = members;
    matchGroupDetails.push(convertToMatchGroupDetail(matchGroup));
  }
...
export const getMatchGroupDetailByMatchGroupId = async (
  ...
  let query =
    "SELECT match_group_id, match_group_name, description, status, created_by, created_at FROM match_group WHERE match_group_id = ? LIMIT 1";
  if (status === "open") {
